...
var
  Add: double;
begin
  // Estimates the total time, assuming that the expected time of a single match
  // is 1.0.
  // The coefficients below are generated by machine learning magic. Refer to
  // tools/time_estimation/notebook.ipynb to see how the learning is done.
  // The "//." comments are added to prevent auto formatting
  Add :=
    2.77918644e-03 * Jobs + //.
    8.64226651e-04 * Things + //.
    9.46858137e-01 + //.
    -1.07427060e-01 * Ln(Things) + //.
    -1.38769432e+00 / Jobs + //.
    2.61388592e-02 * Ln(Things) / Jobs + //.
    4.51703118e-01 / Sqr(double(Jobs)) + //.
    -1.17767833e-01 * Ln(Things) / Sqr(double(Jobs)) + //.
    4.56262770e-02 * Things / Sqr(double(Jobs));
  if Add < 0.0 then
    Add := 0.0;
  Result := Add + Things / Jobs;
end;
